...
public class Utils {
	...
	public static Double doubleFormatDouble(double d) {
	 ...
	 * 在不足len位的数字前面自动补零
	 */
	public static String getLimitLenStr(String str, Integer len) {
		if (str == null) {
			return "";
		}
		while (str.length() < len.intValue()) {
			str = "0" + str;
		}
		return str;
	}
	 ...
	 * 
     */	
	public static String getLimitLenStr(java.lang.String str, java.lang.Integer len, java.lang.String replaceValue) {
	 ...
	 * 取字符的前几位 value,n
	 */
	public static String getSubString(String value, Integer i) {
		if (StringUtils.isBlank(value)) {
			return "mmm";
		}
		if (value.length() <= i) {
	...
	public static boolean creatDirs(String dirPath) {
	...
	public static String[] createDateWeaks(int d){
		String[] workdays={"日","一","二","三","四","五","六"};
		String[] s=new String[d];
		for(int i=0 ;i<d;i++){
			Calendar  now=Calendar.getInstance(); 
			now.add(Calendar.DAY_OF_WEEK,i); 
			int day=now.getTime().getDay();
			SimpleDateFormat df=new SimpleDateFormat("MM-dd");
			s[i]=df.format(now.getTime())+"("+workdays[day]+")";
		}
		return s;
	}
